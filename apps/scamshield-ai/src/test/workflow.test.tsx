...
describe('ScamShield workflow', () => {
  ...
  it('supports caregiver intake and local image evidence preview', async () => {
    ...
    await user.click(await screen.findByRole('button', { name: /analyze suspicious content/i }))
    expect(await screen.findByRole('heading', { name: /create a clear case record/i })).toBeInTheDocument()
  ...
  it('loads a synthetic demo, analyzes it, and exposes timeline, actions, reporting, and export consent', async () => {
    ...
    await user.click(await screen.findByRole('button', { name: /use bank account locked phishing text demo/i }))
    ...
    expect(await screen.findByRole('heading', { name: /risk assessment/i })).toBeInTheDocument()
    ...
    expect(await screen.findByRole('heading', { name: /evidence timeline/i })).toBeInTheDocument()
    ...
    expect(await screen.findByRole('heading', { name: /safe reporting guidance/i })).toBeInTheDocument()
