...
import { ProcessHarborApi } from './api'
import { createSeedRepository } from './repository'

describe('ProcessHarbor Pro health API', () => {
  it('reports reviewer-friendly deployment and scope metadata', () => {
    const api = new ProcessHarborApi(createSeedRepository(), { PROCESSHARBOR_AI_ENABLED: 'false' })
    const health = api.health()

    expect(health.ok).toBe(true)
    expect(health.app).toBe('ProcessHarbor Pro')
