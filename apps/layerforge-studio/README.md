...
LayerForge Studio is configured for Vercel through `vercel.json`. A production Vercel alias is not claimed until a preview and production deployment are verified.
...
- Create documents with custom width, height, and background color.
...
- Autosave after unsaved changes.
...
src/
  ...
  engine/render/       Canvas renderer, compositor, tile-cache extension point
...
Full-canvas rerendering is used for the MVP. `TileCache` is included as an extension point for dirty-region invalidation and tile rendering in a larger-document version.

## Tool and History Design

`ToolController` registers tool classes for brush, eraser, rectangular selection, and workspace pan. Tool classes receive normalized pointer events and call editor runtime methods, keeping pointer behavior separate from React.

History uses command objects. Pixel-changing commands store affected bounds plus before/after `ImageData` patches instead of replacing the entire document. Layer and selection commands store the metadata needed to reverse operations.
...
Projects are saved to IndexedDB under the `layerforge-studio` database. Each project record stores JSON metadata and raster-layer PNG blobs. Loading reconstructs canvas layers from those blobs so editing can continue after refresh.

Browser storage is device- and profile-specific and is not encrypted, cloud-synchronized, or guaranteed durable.
...
- Selection display uses rectangular bounds; selection data is alpha-mask based for future soft selections.
- Large-canvas performance is appropriate for MVP-sized documents; tile rendering and workers are future work.

## Roadmap

1. Layer masks, mask painting, invert/feather/apply, and mask overlay.
...
4. Advanced brush presets, texture stamps, stabilization, pressure curves, and symmetry.
...
7. Portfolio Mode showing source image, layers, major steps, and final export.
...
```text
Project name: layerforge-studio
Root Directory: apps/layerforge-studio
...
No server-side environment variables are required for the documented local editor.

Use a preview deployment first and verify:

1. new-document creation,
2. image import,
3. layer add, rename, reorder, visibility, locking, opacity, and blend modes,
4. brush, eraser, selection, filters, undo, and redo,
5. IndexedDB save/load and autosave,
6. PNG and JPEG export,
7. direct-route refresh and static assets,
8. large-canvas behavior and mobile layout,
9. browser console and storage recovery.

Promote only after Vercel reports `READY` and the editing smoke workflow succeeds.
