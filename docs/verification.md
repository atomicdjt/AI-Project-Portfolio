...
| OpsPilot Pro | `apps/opspilot-ai-operations-toolkit` | `5177` | `npm run build --workspace apps/opspilot-ai-operations-toolkit` | Yes | Yes, `typecheck` script | Yes, Vitest API tests | Yes, Playwright workflow plus proof script | Live Netlify demo documented: `https://opspilot-ai-operations-toolkit.netlify.app/` | Deterministic local drafting engine plus seeded reference Function API, Zod validation, SQL schema, audit/export flow, and documented production adapter path. |
