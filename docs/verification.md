...
| ProcessHarbor Pro | `apps/opspilot-ai-operations-toolkit` | `5177` | `npm run build --workspace apps/opspilot-ai-operations-toolkit` | Yes | Yes, `typecheck` script | Yes, Vitest API/AI/normalization tests | Yes, Playwright workflow plus proof script | Live Netlify demo documented: `https://opspilot-ai-operations-toolkit.netlify.app/` | Deterministic local drafting engine plus seeded reference Function API, optional server-side AI route with validated fallback, Zod validation, SQL schema, audit/export flow, and documented production adapter path. |
...
| ProcessHarbor | 200 | `https://opspilot-ai-operations-toolkit.netlify.app/` |
...
- On 2026-07-04, `curl.exe -L -s -o NUL -w "%{http_code} %{url_effective}"` returned HTTP 200 for the GitHub profile, profile README repo, portfolio repo, Portfolio Hub, LayerForge Studio GitHub Pages subpath, BuildWorld AI, RedactReady Pro, ScamShield AI, RedactReady, ProcessHarbor, FocusForge, VariantVision Pro, Amino Acid Workbench, GardenGrid, and HearthLink public URLs.
