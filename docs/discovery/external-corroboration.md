...
| Grid Dynamics Rosetta PR #319 | **B** | A Rosetta reviewer independently reproduced David's test and type-check numbers, ran ten refutation attempts, found no surviving or newly introduced bypass, and wrote that the analysis was sound. The reviewer also identified David's live-path `DatasetLookup` ambiguity hardening as the valuable half of the change, while requesting deletion rather than hardening of an obsolete `team` authorization path. David subsequently removed the dead team-policy path and related active documentation while retaining and strengthening the live `DatasetLookup` hardening. | The formal review state remains `CHANGES_REQUESTED`. The requested scope correction is present in the current open PR, but there is no subsequent approval or merge yet. The defensible lifecycle state is **requested changes addressed; awaiting maintainer re-review**. |
| Grid Dynamics Rosetta PR #320 | **B** | A Rosetta reviewer independently compared the matcher across roughly 5.5 million inputs with zero divergences, mutation-checked the harness, reproduced the O(n²) → O(n) performance improvement, and found the approach better than the issue's suggested alternatives. The reviewer requested four cross-line regressions, a same-segment later-candidate case, and explanatory comments; those requested additions are present in the current PR along with expanded validation. | The formal review state remains `CHANGES_REQUESTED`. The requested corrective work is present, but the PR has not subsequently been approved or merged. The defensible lifecycle state is **independently corroborated; requested changes addressed; awaiting maintainer re-review**. |
...
- Separate open Rosetta contributions have undergone unusually deep independent equivalence, performance, refutation, and regression analysis; the requested corrective work is present, while formal approval remains pending re-review.
