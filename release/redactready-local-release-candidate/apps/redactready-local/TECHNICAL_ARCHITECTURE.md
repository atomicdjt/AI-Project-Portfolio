...
- `npm run test` runs Vitest coverage for detectors, redaction export/report behavior, adaptive checklist behavior, metadata summaries, and browser-capability helpers.
- `npm run build` runs the TypeScript project build and Vite production build.
- `npm run e2e` runs the Playwright smoke path against a synthetic fixture.
- Manual QA remains required for visual placement, OCR quality, export inspection, responsive layout, and accessibility basics.
