...
test('uploads a text fixture and previews redacted output', async ({ page }) => {
  ...
  await expect(page.getByTestId('detection-list')).toContainText('Possible email address')
  await expect(page.getByTestId('detection-list')).toContainText('Possible SSN-like identifier')
